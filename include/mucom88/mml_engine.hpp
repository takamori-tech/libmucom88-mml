...
public:
    ...
    void advance(uint32_t frameCount)
    {
        ...
        while (m_globalSampleAccum >= 16) {
            ...
            while (m_timerBCount <= 0) {
                ...
                for (int ch = 0; ch < MAX_MML_CHANNELS; ch++) {
                    auto& st = m_channels[ch];
                    if (st.events.empty()) continue;
                    // パート独立ループ: このチャンネルがイベント末尾に達したら
                    // ループポイントに巻き戻す（Z80: 0xFD END検出→ループアドレスにジャンプ）
        ...
        // 全チャンネル終端 → 停止判定
        // パート独立ループ対応: ループポイントを持つチャンネルは自動巻き戻しで
        // ここには到達しない。ループポイントのないチャンネルのみがallDone判定対象。
        bool allDone = true;
        for (int ch = 0; ch < MAX_MML_CHANNELS; ch++) {
            if (m_channels[ch].events.empty()) continue;
            if (m_channels[ch].hasLoopPoint) continue;  // 独立ループ済み
        ...
        if (allDone) {
            if (m_loop) {
                // ループポイント（Lコマンド）があれば、そのtick/eventIdxから再開
                // なければ先頭（tick=0, eventIdx=0）から再開
                uint32_t loopTick = 0;
                bool anyLoopPoint = false;
                for (int ch = 0; ch < MAX_MML_CHANNELS; ch++) {
                    if (m_channels[ch].hasLoopPoint) {
                        anyLoopPoint = true;
                        if (m_channels[ch].loopTick > loopTick)
                            loopTick = m_channels[ch].loopTick;
                        break;
                    }
                }
